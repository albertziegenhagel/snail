<script lang="ts">
    ...
    import type { ProcessInfo, FunctionId, FunctionNode, SampleSourceInfo } from "../utilities/types";
    ...
    function loadMore() {
        ...
        waitingForMore = true;
        const nextPageIndex = functions === null ? 0 : Math.ceil(functions.length / pageSize)
        vscode.postMessage({
...
{#if expanded && process !== null}
    {#if functions !== null}
        {#each functions as func}
          <FunctionTableRow
            on:navigate={(event) =>
                navigateToFunction(event.detail.functionId)}
            func={func}
            isHot={false}
            isActive={activeFunction != null && process.key == activeFunction.processKey && func.id == activeFunction.functionId}
            sampleSources={sampleSources}
          >
          <span slot="function-name-prefix" class="function-name-prefix">
              <div
          ...
          <tr>
            <td>
              <div class="function-name">
                ...
                <span class="load-more"><a href="#top" on:click={() => loadMore()}>more</a></span>
                <span class="load-all"><a href="#top" on:click={() => loadAll()}>all</a></span>
              </div>
            </td>
          </tr>
        {/if}
